...
from google.appengine.api import files, images, mail, users
...
class UploadHandler(webapp2.RequestHandler):
    ...
    def send_mail(self, filename, data):
        mail_address = self.get_mail_address()
        if mail_address is None:
            logging.info('not logged in')
            return

        logging.info('mail to %s', mail_address)
        
        message = mail.EmailMessage(sender=mail_address,
                                    subject=filename)
        message.to = mail_address
        message.body = ""
        message.attachments = [(filename, data)]
        message.send()
    ...
    def options(self):
        logging.info('UploadHandler options()')
        pass

    def head(self):
        logging.info('UploadHandler head()')
        pass

    def get(self):
        logging.info('UploadHandler get()')
        user = users.get_current_user()
        if user is None:
    ...
    def delete(self):
        logging.info('UploadHandler delete()')
